...
public class Control{
    ...
    public Control(Snake game, GUI gui){
        this.gui=gui;
        this.snake=game;
    }
    ...
    public static void main (String[] args){
        ...
        GUI gui = new GUI();
        Control controller = new Control(snakegame, gui);
        gui.registerGUI(controller);    //rekisteröidään tämä ohjainluokka gui:lle
    }
    ...
    public void setCoord(int x, int y){ //asetetaan uusi koordinaatti guihin ja pelin sisäiseen logiikkaan
        ...
        if (gui.autopilot){
            if (!gui.PathFound)
                gui.initPathfind(getFoodX(),getFoodY());
            if (gui.PathFound)
                gui.TraversePath();
        }
    }
